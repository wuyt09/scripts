...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    use comsrf,             only: tpmask, heatforc 
    use pio,                only: file_desc_t
    use ncdio_atm,          only: infld
    ...
    type(file_desc_t) :: fh_tpmask, fh_heat
    logical :: found=.false.
    integer :: nm   ! time frame controller
    character(len=256) :: bnd_tpmask_loc, heat_loc   ! filepath of heat file
    ...
    do nm = 1,12
        call infld('heat', fh_heat, 'lon', 'lev', 'lat', 1, pcols, 1, pver, begchunk, endchunk, &
              heatforc(:,:,:,nm), found, grid_map='PHYS', timelevel=nm)
    end do
    !*** MOD END: infld TP mask and heating file 
...
subroutine tphysac (ztodt,   cam_in,  &
    ...
    integer :: ii, kk, j  ! column indices, level indices, timeframe
    ...
    if ( waccmx_is('ionosphere') .or. waccmx_is('neutral') ) then
       ...
       !***MOD START***********************
       ! set vertical diffusion tend 0
    !*** MOD START: Change heating profile over TP 
        ...
        !do j = 1,12
            do kk = 1,pver
                do ii = 1, ncol
                    if(tpmask(ii,lchnk) > 0.5 ) then ! TP area
                        if (abs(heatforc(ii,kk,lchnk,j)+999.0) > 100.) then
                            ptend%s(ii,kk) = 0._r8 * ptend%s(ii,kk)
                        end if
                    end if
                end do
            end do
    ...
    !do j = 1,12
        do kk = 1,pver
            do ii = 1, ncol
                ...
                if(tpmask(ii,lchnk) > 0.5 ) then ! TP area
                    if (abs(heatforc(ii,kk,lchnk,j)+999.0) > 100.) then
                        ptend%s(ii,kk) = heatforc(ii,kk,lchnk,j) * cpair
                        !write(iulog,"(A5,F6.2,A5,F6.2,A5,F6.2,A15,F8.2)") "Lat:",stat_lat,"Lon:",stat_lon,"Lev:",kk, "heat=",heatforc(ii,kk,lchnk,j)*86400.
                        ! Below is debug info
                        if ((calday-floor(calday))<0.01) then 
                            if ((abs(stat_lon-90.0)<0.5).and. &
                                (abs(stat_lat-35.0)<0.5)) then
                                 write(iulog,"(A30,F8.2,A5,F6.2,A5,F6.2,A5,I6.2,A15,F12.4)") "TRUE Calday:",calday,&
                                 "Lat:",stat_lat,"Lon:",stat_lon,"Lev:",kk,"heat=",heatforc(ii,kk,lchnk,j)*86400.
                                  !exit
                             end if
                        end if
                    end if
