...
subroutine phys_init( phys_state, phys_tend, pbuf2d, cam_out )
    ...
    logical :: found=.false.
    character(len=256) :: bnd_tpmask_loc
    !*****MOD END*****

    !*****MOD START*****
    !fh_tpmask=>tpmask_file_get_id()    
    bnd_tpmask_loc = "/users/yangsong3/wuyt/B/TP_NOSH/TPmask_C20190304.nc"
...
subroutine tphysac (ztodt,   cam_in,  &
    ...
    do i = 1,ncol
        ...
        if (tpmask(i,lchnk) > 0.5) then
            !close sensible heat flux
            cam_in%shf(i) = 0._r8*cam_in%shf(i)
