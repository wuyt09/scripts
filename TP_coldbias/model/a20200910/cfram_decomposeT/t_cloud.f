...
***********************data input*************************************
     ...
     & form='unformatted', access='direct',recl = xt*yt )
      ...
      do k = 1,z1,1
        do i = 1,xt,1
            do j = 1,yt,1
            tem_a(i,j,k) = tem_ax(i,j,k) + par_t(i,j,k,3)
c            print*,i,j,k,tem_a
            end do
        end do
      end do
...
*     OUTPUT
*
       open ( unit = 22, file = './t_cloud_radranc_1.grd',
     & form='unformatted', access='direct',recl= ix*iy )
       open ( unit = 32, file = './t_cloud_radsfc_ranc_1.grd',
     & form='unformatted', access = 'direct',recl = ix*iy )

       open ( unit = 51, file = './t_cloud_input_1.dat',
     & form='unformatted', access = 'direct', recl = ix*iy )
