...
!partial temp. output
     ...
     & form='unformatted', access='direct',recl=IX*IY )
       ...
       open ( unit = 21, file = './drdt_ranc_1.dat',
     ...
     & form='unformatted', access='direct',recl=IX*IY )
       ...
       open ( unit = 35, file = './data/ssr_base.dat',
     & form='unformatted', access='direct',recl=IX*IY )

       open ( unit = 36, file = './data/ssr_warm.dat',
     & form='unformatted', access='direct',recl=IX*IY )

       open ( unit = 37, file = './data/str_base.dat',
     & form='unformatted', access='direct',recl=IX*IY )

       open ( unit = 38, file = './data/str_warm.dat',
     & form='unformatted', access='direct',recl=IX*IY )

       open(51,file = './partial_T_SR.grd',
     & form='unformatted', access = 'direct', recl = IX*IY)

       open(52,file = './forcing_SR.grd',
     & form='unformatted', access = 'direct', recl = IX*IY)
 ...
 100   continue
       ...
       read(35,rec=1)ssrflx_base
       read(36,rec=1)ssrflx_warm
...
c          print*, "after cloud"
     ...
     &                    +((ssrflx_warm(i,j)-ssrflx_base(i,j))
     &                    +(strflx_warm(i,j)-strflx_base(i,j)))
          ...
          enddo
          fc_sfc_dyn(i,j,nv1)=-((ssrflx_warm(i,j)-ssrflx_base(i,j))
     &                        +(strflx_warm(i,j)-strflx_base(i,j)))
