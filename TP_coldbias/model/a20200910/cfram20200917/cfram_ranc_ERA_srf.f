      ...
      INTEGER,PARAMETER :: zd1=37,IX=240,IY=121,zd18=38
...
c  input data
      ...
      real ssr_base(ix,iy),str_base(ix,iy)
      real ssr_warm(ix,iy),str_warm(ix,iy)
...
!forcing output
      ...
      real fc_solar(ix,iy,zd18),fc_err(ix,iy,zd18)
...
!partial temp. output
     ...
     & form='unformatted', access='direct',recl=240*121 )
       ...
       open ( unit = 41, file = './ssr_base.dat',
     & form='unformatted', access='direct',recl=240*121 )

       open ( unit = 42, file = './ssr_warm.dat',
     & form='unformatted', access='direct',recl=240*121 )

       open ( unit = 43, file = './str_base.dat',
     & form='unformatted', access='direct',recl=240*121 )

       open ( unit = 44, file = './str_warm.dat',
     & form='unformatted', access='direct',recl=240*121 )

       open(51,file = './partial_T_1_ERA_SRF.grd',
     & form='unformatted', access = 'direct', recl = 240*121)

       open(52,file = './forcing_1_ERA_SRF.grd',
     & form='unformatted', access = 'direct', recl = 240*121)
 ...
 100   continue
       ...
       read(41,rec=1)ssr_base
       read(42,rec=1)ssr_warm
       read(43,rec=1)str_base
       read(44,rec=1)str_warm
...
c          print*, "after cloud"
          ...
          do k = 1, nv1
             ...
             fc_err(i,j,k)=fc(k)
          enddo
          fc_err(i,j,nv1)=-(rht_warm(i,j,nv1)-rht_base(i,j,nv1))
     &                    +((ssr_warm(i,j)-ssr_base(i,j))
     &                    +(str_warm(i,j)-str_base(i,j)))

          fc(nv1)=fc_err(i,j,nv1)
          call delt_gauss(drdt,fc,nv1)
          do k = 1, nv1
             dt_err(i,j,k)=fc(k)
          enddo
          ...
          enddo
          fc_sfc_dyn(i,j,nv1)=-((ssr_warm(i,j)-ssr_base(i,j))
     &                        +(str_warm(i,j)-str_base(i,j)))
     ...
     &                        -(lhflx_warm(i,j)-lhflx_base(i,j))      
          ...
          enddo
           ...
           dt_err(i,j,zd18)=dt_err(i,j,nv1)
           fc_err(i,j,zd18)=fc_err(i,j,nv1)
           IF(nv.LT.zd1) then
             do l=nv1,zd1
                dt_albedo(i,j,l)=-999
                dt_wv(i,j,l)=-999
 ...
 2000  continue
       ...
       call out3d(dt_err,ix,iy,zd18,51,irec)
       call out3d(fc_err,ix,iy,zd18,52,irec)
