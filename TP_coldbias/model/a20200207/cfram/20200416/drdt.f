      ...
      plev = (/10.,20.,30.,50.,70.,100.,150.,200.,250.,300.,350.,400.,
     &       450.,500.,550.,600.,650.,700.,750.,800.,825.,850.,900., 
     &       925.,950.,1000./)
***********************data input*************************************
      open ( unit = 11, file = '../data/raw/solarin_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 12, file = '../data/raw/ssrd_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 13, file = '../data/raw/ssru_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 14, file = '../data/raw/t2m_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 15, file = '../data/raw/huss_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 16, file = '../data/raw/sp_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 17, file = '../data/raw/o3_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 18, file = '../data/raw/cc_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 19, file = '../data/raw/clwc_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 110, file = '../data/raw/ciwc_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 111, file = '../data/raw/hus_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
      open ( unit = 112, file = '../data/raw/t_base.dat',
     & form='unformatted', access='direct',recl = xt*yt )
...
c      real tas(IX,IY),huss(IX,IY),rlus(IX,IY)
      ...
      real r0k_tot(100),drdt_tot(100,100)
...
*     OUTPUT
*
       open ( unit = 52, file = './base_no_cloud_out_1.dat',
     & form='unformatted', access = 'direct', recl = 100*100 )
       open ( unit = 53, file = './drdt_ranc_1.dat',
     & form='unformatted', access='direct',recl=100*100)
...
c             pmean=0.5*(pp(l)+pp(l+1))
 ...
 ! cloudy sky calculation
     ...
     &            lw_base,water_c,ice_c,iseed,no_cloud_out)
           ...
           do l = 1, nv
             tot=fuir_tot(l+1)-fuir_tot(l)
             rad_base(l)=fdir_tot(l)-fdir_tot(l+1)+tot
           enddo
           rad_base(nv1)=fdir_tot(nv1)-fuir_tot(l)

           if( k0 .eq. 0)then
              DO l=1,nv1
                 r0k_tot(l)=-rad_base(l)
              ENDDO
           else
            do l = 1, nv1
             drdt_tot(l,k0)=-rad_base(l) - r0k_tot(l)
            enddo
           endif
